...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Name for the log file to be created in the main output directory."
    )
    try:
    ...
    if RICH_AVAILABLE: # pragma: no cover
        ...
        # For RichHandler, the formatter is often best left to Rich's defaults for markup,
        # or set a simple one if specific non-markup format is needed.
        # console_formatter = logging.Formatter("%(message)s", datefmt="[%X]") # Already there
        # console_handler.setFormatter(console_formatter) # Already there
    else: # pragma: no cover
    ...
    app_logger = logging.getLogger("pycodon_analyzer")
    if app_logger.hasHandlers(): # pragma: no cover
        # Clear existing handlers if any (e.g., if main() is called multiple times in a test)
        for h in list(app_logger.handlers):
            app_logger.removeHandler(h)
            h.close()

    app_logger.addHandler(console_handler)
    # Set the logger level to the lowest of its handlers (DEBUG if verbose, otherwise INFO for console)
    # The FileHandler will also have its own level.
    app_logger.setLevel(logging.DEBUG) # Set app logger to DEBUG to capture all messages for the file log
    app_logger.propagate = False # Prevent messages from going to the root logger if it has handlers

    # --- ADDED: File Handler Setup ---
    # The log file should be placed in the output directory.
    # This means we need to determine output_dir_path early,
    # or at least know it before executing the command function.
    # The `args.output` is available for `analyze`, `args.output_dir` for `extract`.

    log_file_path: Optional[Path] = None
    if args.command == "analyze":
        # For analyze, output_dir_path is determined by args.output
        # We need to ensure _setup_output_directory is called before this if it creates the dir.
        # Or, we can create it here if it doesn't exist.
        # Let's assume args.output is the root for the log file.
        output_dir_for_log = Path(args.output)
        output_dir_for_log.mkdir(parents=True, exist_ok=True) # Ensure output dir exists for the log file
        log_file_path = output_dir_for_log / args.log_file
    elif args.command == "extract":
        # For extract, output_dir_path is args.output_dir
        output_dir_for_log = Path(args.output_dir)
        output_dir_for_log.mkdir(parents=True, exist_ok=True) # Ensure output dir exists
        log_file_path = output_dir_for_log / args.log_file
    ...
    if log_file_path:
        try:
            ...
            # Compact, but still informative, format
            file_formatter = logging.Formatter(
                '%(asctime)s [%(levelname)-7s] %(name)-22s : %(message)s (%(funcName)s L%(lineno)d)',
                # Example of an even simpler format for INFO if you use LevelSpecificFormatter later:
            ...
            app_logger.info(f"File logging initiated to: {log_file_path}")
        except Exception as e: # pragma: no cover
            # Fallback to console if file logging setup fails
            console_error_logger = logging.getLogger("pycodon_analyzer.log_setup_error")
            # Ensure this temporary logger doesn't propagate if app_logger is set not to.
            # Add console_handler directly if not already on root or if app_logger is cleared.
            if not console_error_logger.hasHandlers(): # Add console handler if it doesn't have one
                temp_console_handler = logging.StreamHandler(sys.stderr)
                temp_console_formatter = logging.Formatter(
                    '%(asctime)s - %(levelname)s - %(name)s - %(message)s'
                    )
                temp_console_handler.setFormatter(temp_console_formatter)
                console_error_logger.addHandler(temp_console_handler)
            console_error_logger.error(f"CRITICAL: Failed to set up file logging to {log_file_path}: {e}")
